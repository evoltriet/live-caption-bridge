...
test('captures injected PCM and drives screen, OBS, and native overlay outputs', async () => {
  ...
  try {
    ...
    if ((await page.getByRole('heading', { name: 'Live Caption Bridge' }).count()) === 0) {
      throw new Error(
        `Operator renderer did not mount. URL=${page.url()} diagnostics=${diagnostics.join(' | ')} HTML=${(await page.content()).slice(0, 1_000)}`
      )
    }
    await expect(page.getByRole('heading', { name: 'Live Caption Bridge' })).toBeVisible()
    ...
    let nativeOverlay:
      | { alwaysOnTop: boolean; focusable: boolean; url: string }
      | undefined
    await expect
      .poll(async () => {
        nativeOverlay = await application.evaluate(({ BrowserWindow }) => {
          const window = BrowserWindow.getAllWindows().find((candidate) =>
            candidate.webContents.getURL().includes('mode=native')
          )
          return window
            ? {
